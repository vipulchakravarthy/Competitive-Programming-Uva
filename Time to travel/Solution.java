		...
		while(scan.hasNextLine()){
			...
			for(int i = 0; i < vertices; i++){
				...
				for(int j = 1; j < tokens.length-1;j += 2){
					graph.addEdge(new DirectedEdge(i,Integer.parseInt(tokens[j])-1, Integer.parseInt(tokens[j+1])));
				}
 			}
            ...
            LazyDijkstraSP ld = new LazyDijkstraSP(graph, Integer.parseInt(input[0])-1);
            System.out.println(ld.pathTo(Integer.parseInt(input[1])-1));
            Iterable<DirectedEdge> iter = ld.pathTo(Integer.parseInt(input[1])-1);
            String st = "Case " + cases +": Path = " + input[0] +" ";
            for(DirectedEdge de: iter){
                st += (de.to()+1) +" ";
            }
            st = st.substring(0,st.length()-1);
            st += "; " + ((int) ld.distTo(Integer.parseInt(input[1])-1)) + " second delay";
