...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < N; i++){
            ...
            int baseAddr = Integer.parseInt(st.nextToken());
            int size = Integer.parseInt(st.nextToken());
            int dimension = Integer.parseInt(st.nextToken());
            int[] boundaries = new int[2*dimension];
            for(int j = 0; j < boundaries.length; j++){
                boundaries[j] = Integer.parseInt(st.nextToken());
            }
            //storing details of each array into sep class
            PhysicalAddress phy_arr = new PhysicalAddress(baseAddr,size,dimension,boundaries);

            map.put(name,phy_arr);
        }

  //       for (Map.Entry<String, PhysicalAddress> entry : map.entrySet()) {
...
class PhysicalAddress {
    ...
    PhysicalAddress(int baseAddr, int size, int dimension, int[] b){
